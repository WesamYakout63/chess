...
char board[8][8],capturedpiece[8][4],garbage,color,array[8][4],pormot,Exit=0;
...
\"l\":To LOAD the game\n!-->PLEASE NOTE THAT ANY INCREMENT THAN THE REQUIRED WILL BE NEGLEGTED<--!\n\n");
    ...
    if(STALEMATE==1)
    {
        if(board[1][3]=='P' &&board[1][4]=='P' &&board[1][5]=='P')
        {
            printf("Game Over!\n\t\t DRAW;;black Player stalemate ");
            Exit=1;
        }
    ...
    {
        if (board[6][3]=='p' &&board[6][4]=='p' &&board[6][5]=='p')
        {
        printf("Game Over!\n\t\t DRAW;;white Player stalemate");
        Exit=1;
    }
    }
    else if(checkmate==1)
